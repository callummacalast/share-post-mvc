...
class Users extends Controller
{
  ...
  // Handles post request and register form
  {
    // Check for post
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
      ...
      if (empty($data['password'])) {
        ...
        $data['password_err'] = 'Please must be atleast 6 characters';
      }
      ...
      if (empty($data['email_err']) && empty($data['name_err']) && empty($data['password_err']) && empty($data['confirm_pass_err'])) {
        ...
        if($this->userModel->register($data)){
          flash('register_success', 'You have been successfully registered! Please log in to continue');
